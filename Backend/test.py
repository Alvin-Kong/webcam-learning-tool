...
class ObjectTracker():
    ...
    def drawObjectBox(self, frame, bbox):
        x = int(bbox[0])
        y = int(bbox[1])
        width = int(bbox[2])
        height = int(bbox[3])
    ...
    def getCenter(self, bbox):
        xcenter = self.getXCenter(bbox)
        ycenter = self.getYCenter(bbox)
    ...
    def runTracker(self):
        ...
        while(self._capture.isOpened()):
            ...
            if self._ret:
                self.drawObjectBox(self._frame, self._bbox)
